...
It is a script rather than a slash command by design: this has to run at a
...
| `verify_rules.py` | 988 cross-checks of the printed Player's Book against `chargen.json` against the spine formula: the Calling tables, the arms table, the **feature prose, the 3rd-level paths and the picker blurbs** beside them, and since 2026-08-22 **Ch. IV's encounter budget** in both Keeper-side books against `Rules.BudgetRungs`. The guard the whole project rests on. | instant | `blood-and-grit.html`, `keeper-handbook.html`, `bestiary.html`, `legends.html` |
| `audit_consistency.py` | 105,958 cross-checks of the **Keeper's** side, which `verify_rules.py` does not reach: Threat by Tier and Sign & Spoor across book/app/`CLAUDE.md`, `creatures.json` still current with the built Bestiary, the Roll-by-Tier appendix, all 143 Grounds entries, every condition a creature inflicts defined in Appendix B, the printed benchmarks against the 182 creatures that are supposed to match them, every typed copy of that count against the count itself, one shared vocabulary across every built book, every chapter cross-reference, **app↔book feature parity**, one sample county the four books cannot disagree about, and every number the two front pages spell out in words, against the file that carries it. Anything one carries that the other does not. | instant | every built book, `Core.cs`, `CharGen.cs`, `creatures.json`, both READMEs |
| `audit_diversity.py` | Is every possibility the rules offer one some path can reach? Fails only on **dead surface**. A skill no Calling wants and no Origin grants, a condition nothing inflicts, a power list that stops short of rank 5, an ability no Origin raises. Everything else (the Bestiary's Tier grid, the Dread spread, how many ways a thing can be put down, what share of the Bestiary the Grounds tables reach) is measured and printed for a designer, and never touches the exit code. | instant | every built book, `chargen.json`, `creatures.json` |
...
| `audit_ai_tells.py` | The prose reads as a person wrote it. Fails on hard tells, negative parallelism above all. Also prints research signals for every file beside two texts written without a model (the 5e SRD and *The Virginian*): Claude's own habits, the Antislop phrases, what StoryScope found in AI fiction, lexical richness, participial clauses, contractions and dashes. `--books` reads every built book, boxes included and stat blocks left out. `--strict` also fails on the research shapes and on any rate running past twice the human baseline. `--worklist F` writes every finding to a file. `--commits N` reads the last N commit messages. Word lists and sources are in `ai_tells_lexicon.py`. | seconds | git history for `--commits` |
