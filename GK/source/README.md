...
(Player's Book v2.54 · Keeper's Book v2.38 · Bestiary v2.25).
...
used to silently regenerate a *different* countryside, because every feature drew from
...
- **🧭 Wizard…**: build a custom character choice by choice through nine steps: level,
  ...
  generator.
- **🎲 New look**: draw the description again and leave every number alone. All of it
...
- **A fresh start everywhere**: every roster and record has its own clear (Clear posse, Clear field, New fight, Clear (encounter), Clear log, Clear (generators),
...
- **Keyboard-first**: Ctrl+1–9 switches tabs and Ctrl+0 reaches the tenth; Enter rolls
  ...
  map; Left/Right turn the Reference deck. Destructive clears stay
  click-and-confirm.
- **The emblem, everywhere it should be**: the steer-skull-and-rifles emblem is
  ...
  watermark in the dead space of the busier tabs, drawn only when there's real
