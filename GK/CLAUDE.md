...
suppresses re-capture during its own bulk rebuild so a restore is a single step.
...
- **Session notes DO capture, but only on `Leave`.** The original reasoning (a
...
what each tab *is*, plus the decisions already settled.
...
- **Tracker**: initiative, rounds, damage/heal (two-way synced with Posse), conditions,
  ...
  **Nerve and the Mark ride beside Blood** (v1.57.0): read-only, unbound, drawn from the soul
  ...
  Several things here hold the rest up:
...
- **Map**: seeded procedural frontier surveys: ground × scale × hour × water, with
  ...
  **clipped to the inner neatline at generation** (`ClipPolyline`). They are
...
- **New Soul**: a strictly-by-the-book character maker: Ch. III's eight steps at any level 1–10,
  both ability methods, all 18 Callings and 16 Origins with their cross-constraints honored.
...
- **Generators**: every Ch. XII rollable table plus all nine Grounds terrain tables and the Hand
  ...
  real creature (the smoke suite asserts it). **The White Bison stays off every table by design**,
...
`EnableCompressionInSingleFile` and `PublishReadyToRun` are **off** for startup speed
...
cannot cross that line, so this means a second front-end against the same library; nothing needs porting.
...
- **`Upright` cannot be derived.** At 0 Blood `Down` is true and `CanAct` refuses the turn, so
  ...
  round" means. It does not stop the bleeding: the book prices consciousness in Grit.
...
`ResetForNewFight` clears **none** of it except `Upright`: dying is Blood, and Blood
...
pale colour toward white, which is how the old palette failed: `PcRow` and a near-white
...
Ch. XI was audited against the engine and nine gaps closed. Six decisions already settled:
...
matches. Sentences about something ongoing are left alone: *"allies inside recover
...
`FeatureCadence` is ordered by design (Turn, Round, Scene, Dawn, Trigger, Session) because the
...
Two seams to know about, both found by writing the reader:
...
behind the comment vanishes from the audit, so keep prose comments above the call.

## What a soul OWES is not a ration (v1.44.0)

The Calling strip counts two different things and draws them differently by design.
...
Five decisions already settled:
...
- **A fact that is only printable stops being true.** The sheet said "+2 Notice" and every number
  ...
  `uses` column: whenever one of a pair of facts is only printable, the other one goes silently
  wrong.
- **The familiar is a FOURTH CAST, and not `IsPC`.** `Combatant.FamiliarOf` holds its
...
- **The rite is an act the Witch performs.** "Sickened until you can bind another over a long night's
  rite" is `BindNewFamiliar`, on the Calling strip and the row menu. It is NOT hung on
  Rest or on any dawn: every other boundary in this app hands something back because a clock turned
  over, and a Sickened that lifted by itself overnight would silently say the loss cost nothing.
...
stays spent. The Craft's two levels come from `CharGen.SubpathLevels`, read off the level table (3rd and 9th for the three of the Old Dark, 10th for everybody else), so no familiar rule carries a
literal 3 or 9.
...
The plain reading of the books before this: nothing did. The Faith Callings pay for Miracles out
...
- **Mending at the last rung is warned about and PERMITTED.** `WhyNotMend` refuses only at
  Consumed; Hunger 5 gets a `Confirm` and the door stays open. A player bleeding out who spends their
  last step to stay upright one more round is the reason this Origin exists, and the app's
...
- **Numbness spares the Nerve and nothing else.** A numb soul still fails, is still Frightened, and
  ...
  because Nerve is the one that measures still being able to care. The book prints it as a gift and says in the next line that it is not one, and `DreadOutcome.Numb` is what lets the log say so.
...
**The numbers came off the harness.** Spending the budget exactly wiped a posse of
...
`Rules.CanSpendBeats(c, n)` and `Rules.WhyNoBeats(c, n)` are a pair by design: the answer and the
...
`StrikeAndApply` was left alone: `GK/playtest` drives fights through it on a
...
and is left alone, because re-seating on every resize would undo a Keeper's drag. A **measured** one, `preferred` returning `MeasuredColumnWidth(panel)`, for a fixed-width column of buttons, keeps
...
- **Georgia is a text-figure face.** Its 3 4 5 7 9 descend and its 0 1 2 sit at x-height, so "30" looks like "3o" and a column of numbers doesn't line up. GDI+ has no way to request a font's
...
- **`DrawString(text, font, brush, x, y)` has no width and will not stop.** Overflow gets painted
  over by whatever is drawn next, which looks like a truncation with no cause. Draw into a
...
never emptied by design: the set of triples the app draws with is small and fixed, so it settles at a
...
* **First selection** of a tab runs its builder. Once per session, 200–1000 ms. Already deferred
  by design (see the ctor's note). That third of the launch is paid a tab at a time instead.
...
Switch time tracks the control count of a tab at roughly 2 ms a control on this laptop, so the remaining lever is fewer controls per page.
...
had missed. `Clear log` is exempt: the roll log is a record of what happened rather
...
of Segoe UI 10.5 bold, and **the second line rendered nothing at all** from v1.51.0 until this was found, missing outright rather than clipped.
...
columns by design: true of the columns, false of the header band, which is pinned to one line's
...
- **`--selftest`** constructs all three run modes, walks all ten realized tabs and every wizard step
  **for every Calling**, and fails on any interactive control carrying no tooltip. It also
  builds the Reference deck by design (tabs are realized lazily, so nothing else touches it) and
...
the build rather than silently desynchronising book and app.
...
and nothing else. Three choices to keep: it is **opt-in** (`Prefs.Data.TurnTimer`,
...
drops by **√time**, so the *area* the eye takes for "how much is left" falls off linearly. The
...
books' rules. Two rules govern it and both matter.
...
a Norwegian in a charro jacket over mining boots and that sounds like a machine talking. `callingStyles`
...
Four were found by looking for them by design. The fourth, fonts minted on repeating paths, has its
...
2. **`AutoSave` returns whether it landed** and says so once per new reason; swallowing the failure was
   ...
   saving perfectly.
3. Anything that **reparents** a control unwinds in a `finally`. That failure arrives with nothing to read: a tab that stays blank until the app is restarted, and no exception or dialog to say why.
...
`GK/source/README.md`, the root CLAUDE.md). `--selftest` builds the deck by design (tabs are
...
**Three things do NOT:** `startup-error.txt` and `selftest-report.txt`, because a crash
...
if it is a diagnostic about THIS COPY of the exe, it stays beside the exe. Because the state is no longer kept beside the binary, `GritKeeper\app\GritKeeper.exe` is now a perfectly good thing to play from.
...
`selftest-report.txt` out of that folder:, because v1.20.1 shipped with the packager's
...
stays on its old build until closed, and passing `-Staged` takes that path from the start.
